...
//  ABGaugeViewDemo
//
//  Created by Ankita Thakur on 29/09/24.
//
...
public class ABGaugeView: UIView {
    ...
    func drawSmartArc() {
        ...
        for index in 0..<arcColors.count {
            ...
            // Add label at each angle (mid-point of the arc)
            let midAngle = (angles[index] + angles[index+1]) / 2
            addLabelForAngle(angle: midAngle, withText: "\(index+1)")
        }
    ...
    func addLabelForAngle(angle: CGFloat, withText text: String) {
        let radius: CGFloat = max(bounds.width, bounds.height) / 2 - self.frame.width / 8 // Adjust radius if needed
        ...
        let label = UILabel()
        label.text = text
        label.font = UIFont.systemFont(ofSize: 10) // Adjust font size as needed
...
extension Array {
    mutating func rearrange(from oldIndex: Int, to newIndex: Int) {
